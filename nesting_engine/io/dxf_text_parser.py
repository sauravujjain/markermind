"""
Text-label DXF Parser (Gerber-style nested markers).

Extracts closed polylines as piece boundaries and associates
text labels found inside each piece to determine piece names and sizes.

This parser handles DXF files where:
- Pieces are closed POLYLINE/LWPOLYLINE entities in modelspace
- TEXT/MTEXT entities inside each piece identify name, size, pattern ID
- A container rectangle (largest polyline) may represent the marker boundary
- Marker info text like "W=70.000IN L=17.9997 YD U=71.623%" may be present

Gerber format characteristics:
- All boundaries on layers like 'T001L001'
- Text on 'ABC' layer
...
Example:
    >>> from nesting_engine.io.dxf_text_parser import DXFParser
    >>> parser = DXFParser("marker.dxf")
    >>> pieces = parser.to_nesting_pieces()
    >>> for piece in pieces:
    ...     print(f"{piece.identifier.piece_name}: {piece.identifier.size}")
"""
...
class DXFParser:
    """
    Parser for garment pattern DXF files with text labels inside polylines.

    Handles Gerber Technology format and similar text-label DXF variants.
...
def load_pieces_from_dxf(
    ...
    """
    Convenience function to load pieces from a DXF file using text-label parsing.
