...
export default function OrderDetailPage() {
  ...
  const [uploadFileType, setUploadFileType] = useState<'aama' | 'optitex_aama' | 'gerber_aama' | 'gerber_accumark' | 'dxf_only' | 'vt_dxf'>('aama')
  ...
  const handlePatternUpload = async () => {
    ...
    try {
      const newPattern = await api.uploadPattern(
        ...
        uploadFileType,
        selectedDxfFile,
        (uploadFileType === 'aama' || uploadFileType === 'optitex_aama' || uploadFileType === 'gerber_aama') ? (selectedRulFile || undefined) : undefined,
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <div className="mt-2">
              <input
                type="text"
                placeholder="Add a note (e.g., HD FGL pilot 3.5x3.5)..."
                defaultValue={(order as any).notes || ''}
                className="w-full text-xs px-2 py-1 border border-dashed border-muted-foreground/30 rounded bg-transparent text-muted-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:border-primary/50 focus:text-foreground"
                onBlur={async (e) => {
                  const val = e.target.value.trim()
                  if (val !== ((order as any).notes || '')) {
                    await api.updateOrder(order.id, { notes: val } as any)
                    loadData()
                  }
                }}
                ...
                <div className="p-4 border border-dashed border-primary/30 rounded-lg bg-primary/5 space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const val = e.target.value as typeof uploadFileType;
                        setUploadFileType(val);
                        setSelectedRulFile(null);
                        if (val !== 'dxf_only') setSizeNames('');
                      }}
                      className="w-full px-3 py-2 border rounded-md text-sm bg-background"
                    >
                      <option value="aama">AAMA (DXF + RUL)</option>
                      <option value="optitex_aama">OptiTex AAMA (DXF + RUL)</option>
                      <option value="gerber_aama">Gerber AAMA (DXF + RUL)</option>
                      <option value="gerber_accumark">AccuMark (DXF only)</option>
                      <option value="dxf_only">DXF Only (generic)</option>
                      <option value="vt_dxf">VT DXF (Optitex graded)</option>
                    </select>
                    {uploadFileType === 'optitex_aama' && (
                      <p className="text-[11px] text-muted-foreground mt-1">OptiTex AAMA — DXF + RUL with packed grading</p>
                    )}
                    {uploadFileType === 'gerber_aama' && (
                      <p className="text-[11px] text-muted-foreground mt-1">Gerber AccuMark AAMA — DXF + RUL with grading rules</p>
                    )}
                    {uploadFileType === 'gerber_accumark' && (
                      <p className="text-[11px] text-muted-foreground mt-1">Gerber AccuMark expanded shapes — materials from ANNOTATION</p>
                    )}
                    {uploadFileType === 'dxf_only' && (
                      <p className="text-[11px] text-muted-foreground mt-1">Pre-sized pieces — no grading file needed</p>
                    )}
                    {uploadFileType === 'vt_dxf' && (
                      <p className="text-[11px] text-muted-foreground mt-1">Optitex Graded Nest — sizes auto-detected</p>
                    )}
                  </div>
                  ...
                  {(uploadFileType === 'aama' || uploadFileType === 'optitex_aama' || uploadFileType === 'gerber_aama') && (
                  <div>
